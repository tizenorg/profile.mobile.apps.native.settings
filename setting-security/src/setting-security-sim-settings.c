...
static int setting_security_sim_settings_update(void *cb)
{
	...
	if (ad->data_pin_lk) { /*it indicates TAPI is not responsed in time */
		/*setting_update_gl_item_chk_status(ad->data_pin_lk, ad->pin1_status); */
		ad->data_pin_lk->chk_status = ad->pin1_status;
		elm_genlist_item_fields_update(ad->data_pin_lk->item, "*", ELM_GENLIST_ITEM_FIELD_CONTENT);
	...
	if (ad->data_change_pin2 && ad->data_change_pin2->item) {
		if (ad->pin2_blocked_flag) {
			if (!safeStrCmp(ad->data_change_pin2->keyStr, "IDS_ST_BODY_CHANGE_PIN2")) {
				ad->data_change_pin2->keyStr = (char *)g_strdup("IDS_CST_BODY_UNBLOCK_PIN2");
				elm_object_item_data_set(ad->data_change_pin2->item, ad->data_change_pin2);
				elm_genlist_item_update(ad->data_change_pin2->item);
			}
		} else {
			if (!safeStrCmp(ad->data_change_pin2->keyStr, "IDS_CST_BODY_UNBLOCK_PIN2")) {
				ad->data_change_pin2->keyStr = (char *)g_strdup("IDS_ST_BODY_CHANGE_PIN2");
				elm_object_item_data_set(ad->data_change_pin2->item, ad->data_change_pin2);
				elm_genlist_item_update(ad->data_change_pin2->item);
			}
		}
...
static void get_pin_lock_info_cb(TapiHandle *handle, int result, void *data, void *user_data)
{
	...
	int old_state = ad->data_pin_lk->chk_status;/*elm_check_state_get(ad->data_pin_lk->eo_check); */
	if (old_state) {
...
void _draw_pin_onoff_status(void *data, Evas_Object *check)
{
	...
	if (tapi_ret != TAPI_API_SUCCESS) {
		SETTING_TRACE_ERROR("tel_get_sim_type call failed with error code %d", tapi_ret);
		return;
	}
...
static void get_sim_lock_info_cb(TapiHandle *handle, int result, void *data, void *user_data)
{
	...
	int old_state = ad->data_sim_lk->chk_status;/*elm_check_state_get(ad->data_sim_lk->eo_check); */
	if (old_state) {
...
static void get_change_pin_info_cb(TapiHandle *handle, int result, void *data, void *user_data)
{
	...
	SETTING_TRACE_DEBUG("sec_ret[%d], lock_type[%d], lock_status[%d], retry_count[%d]", sec_rt, lock->lock_type, lock->lock_status, lock->retry_count);
	if (lock->lock_type == TAPI_SIM_LOCK_SC && lock->lock_status == TAPI_SIM_LOCK_KEY_PUK) { /* PUK required : 0x02*/
		SETTING_TRACE("Current status of PIN Lock is Blocked");
		ad->pw_type = SETTING_SEC_PW_PIN1_BLOCKED;
...
void _mouse_up_change_pin(void *data, int sel_item)
{
	...
	if (sel_item == 1 && (ad->pin1_status == TAPI_SIM_FACILITY_ENABLED)) {
		...
		if (tel_get_sim_lock_info(ad->handle, TAPI_SIM_LOCK_FD, get_change_pin_info_cb, ad) != 0) {
			SETTING_TRACE_ERROR("failed to call tel_get_sim_lock_info()");
		}
		/*ad->pw_type = SETTING_SEC_PW_CHANGE_PIN2; */
		/*setting_security_create_password_sg(ad); */
													   ...
													   void *event_info)
{
	...
	} else if (!safeStrCmp("IDS_CST_BODY_UNBLOCK_PIN2", list_item->keyStr)) {
		ad->pw_type = SETTING_SEC_PW_PIN2_BLOCKED;
		setting_security_create_password_sg(ad);
	}
}
