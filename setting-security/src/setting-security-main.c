...
static Eina_Bool setting_security_main_click_softkey_back_cb(void *data,
		Elm_Object_Item *it);
...
static int setting_security_main_create(void *cb)
{
	...
	view_init(&ad->md, KeyStr_LockScreen);
	scroller = ad->md.genlist;
	...
	setting_create_navi_bar_buttons(
	       ...
	       ad, scroller, ad->md.navibar_main,
	       NULL);

	/* Group List: Lock screen */
	setting_create_Gendial_field_titleItem(scroller, &itc_group_item,
	...
	setting_create_Gendial_field_def(scroller, &(ad->itc_1text),
			...
			NULL, NULL, 0, KeyStr_LockScreen, NULL, NULL);
	...
	if (ret != APP_MANAGER_ERROR_NONE) {
		...
		ad->data_security_update = setting_create_Gendial_field_def(
				scroller, &(ad->itc_1text_1icon),
				setting_security_main_mouse_up_Gendial_list_cb,
				ad, SWALLOW_Type_INVALID,
				NULL, NULL, value, _(SECURITY_UPDATE_TEXT),
				NULL, NULL);

		if (app_info_destroy(app_info) != 0) {
			SETTING_TRACE_ERROR(
					"app_info_destroy error for org.tizen.msa");
		}
	}
	...
	if (ad->data_sim_settings) {
		if ((ad->handle == NULL) || (isEmulBin() == TRUE)) {
			setting_disable_genlist_item(
					ad->data_sim_settings->item);
		} else {
			ret = setting_get_int_slp_key(INT_SLP_SETTING_SIM_SLOT,
					&value, &err);
			tapi_ret = tel_check_modem_power_status(ad->handle,
					&b_status);
...
static UNUSED Eina_Bool _check_tapi_async_cb_is_called(void *data)
{
	...
	if (!ad->enter_tapi_async_cb_flag) {
		...
		NULL, KeyStr_Security_Waiting_Sim,
				...
				0, FALSE, FALSE, 0);
		ad->remove_sim_popup_timer = ecore_timer_add(1,
				__remove_sim_popup_cb, ad);
	}
	ad->tapi_async_cb_check_timer = NULL;
...
void setting_security_lockscreen_layout_ug_cb(ui_gadget_h ug, enum ug_mode mode,
		void *priv)
{
	SETTING_TRACE_BEGIN;
	Evas_Object *base;
...
static UNUSED void setting_security_lockscreen_destroy_ug_cb(ui_gadget_h ug,
		void *priv)
{
	SETTING_TRACE_BEGIN;
	ret_if(priv == NULL);
