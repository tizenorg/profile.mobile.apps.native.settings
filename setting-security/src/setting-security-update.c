...
static Eina_Bool setting_security_update_click_softkey_back_cb(void *data, Evas_Object *obj, void *event_info);
                                              ...
                                              void *event_info)
{
	SETTING_TRACE_BEGIN;
	/* error check */
	retvm_if(data == NULL, EINA_FALSE, "[Setting > Security update] Data parameter is NULL");
	SettingSecurityUG *ad = (SettingSecurityUG *) data;
	if (&setting_view_security_update == ad->view_to_load) {
		/* Send destroy request */
		ug_destroy_me(ad->ug);
	} else
		setting_view_change(&setting_view_security_update, &setting_view_security_main, ad);
	SETTING_TRACE_END;
	return EINA_TRUE;
}
                                                        ...
                                                        void *user_data)
{
	...
	if (result == APP_CONTROL_RESULT_SUCCEEDED) {
		...
		if (!safeStrCmp(_("AUTO_ON"), resultMsa)) {
			...
			setting_security_update_gl_item_chk_status(ad->data_security_update_chk, 1);
			if (ad->data_security_wifi_update_chk) {
				elm_object_item_disabled_set(ad->data_security_wifi_update_chk->item, EINA_FALSE);
			}
		} else if (!safeStrCmp(_("AUTO_OFF"), resultMsa)) {
			SETTING_TRACE_DEBUG("auto update - off");
			setting_security_update_gl_item_chk_status(ad->data_security_update_chk, 0);
			if (ad->data_security_wifi_update_chk) {
				elm_object_item_disabled_set(ad->data_security_wifi_update_chk->item, EINA_TRUE);
			}
		}
		FREE(resultMsa);
		/*for via wifi value */
		...
		if (!safeStrCmp(_("WIFI_ON"), resultMsa)) {
			...
			setting_security_update_gl_item_chk_status(ad->data_security_wifi_update_chk, 1);
		} else if (!safeStrCmp(_("WIFI_OFF"), resultMsa)) {
			SETTING_TRACE_DEBUG("wifi only - off");
			setting_security_update_gl_item_chk_status(ad->data_security_wifi_update_chk, 0);
		}
		FREE(resultMsa);
...
static void __get_toggle_status_form_msa(void *data)
{
	...
	app_control_set_operation(service, "http://org.tizen.msa/appcontrol/operation/query");

	ret = app_control_add_extra_data(service, "KEY", "STATUS");
	SETTING_TRACE_DEBUG("add extra data (KEY,STATUS) to service ret=%d", ret);

	ret = app_control_send_launch_request(service, security_update_get_status_service_reply_cb, (void *)data);
	SETTING_TRACE_DEBUG("send launch service (KEY,STATUS) request ret=%d", ret);
	if (ret != APP_CONTROL_ERROR_NONE) {
                                                ...
                                                void *user_data)
{
	...
	if (result == APP_CONTROL_RESULT_SUCCEEDED) {
		SETTING_TRACE_DEBUG("APP_CONTROL_RESULT_SUCCEEDED");
		setting_update_gl_item_chk_status(ad->data_security_update_chk, 1);
	} else if (result == APP_CONTROL_RESULT_FAILED) {
		SETTING_TRACE_DEBUG("APP_CONTROL_RESULT_FAILED");
		setting_update_gl_item_chk_status(ad->data_security_update_chk, 0);
	} else if (result == APP_CONTROL_RESULT_CANCELED) {
		SETTING_TRACE_DEBUG("APP_CONTROL_RESULT_CANCELED");
		setting_update_gl_item_chk_status(ad->data_security_update_chk, 0);
	}
                                                 ...
                                                 void *user_data)
{
	...
	if (result == APP_CONTROL_RESULT_SUCCEEDED) {
		...
		if (ad->data_security_wifi_update_chk) {
			elm_object_item_disabled_set(ad->data_security_wifi_update_chk->item, EINA_TRUE);
			elm_object_disabled_set(ad->data_security_wifi_update_chk->eo_check, EINA_TRUE);
		}
	} else if (result == APP_CONTROL_RESULT_FAILED) {
		SETTING_TRACE_DEBUG("APP_CONTROL_RESULT_FAILED");
		setting_update_gl_item_chk_status(ad->data_security_update_chk, 1);
                                               ...
                                               void *event_info)
{
	...
	if (POPUP_RESPONSE_OK == response_type) {
		...
		if (ret != APP_CONTROL_ERROR_NONE) {
			...
			ret = app_control_add_extra_data(service, "KEY", "AUTO_ON");
			SETTING_TRACE_DEBUG("add extra data (KEY,AUTO_ON) to service ret=%d", ret);

			ret = app_control_send_launch_request(service, security_update_on_service_reply_cb, (void *)data);
			SETTING_TRACE_DEBUG("send launch service (KEY,AUTO_ON) request ret=%d", ret);
			if (ret != APP_CONTROL_ERROR_NONE) {
				SETTING_TRACE_ERROR("Cannot launch app");
			}
			if (ad->data_security_wifi_update_chk) {
				elm_object_item_disabled_set(ad->data_security_wifi_update_chk->item, EINA_FALSE);
				elm_object_disabled_set(ad->data_security_wifi_update_chk->eo_check, EINA_FALSE);
			}
		}
		app_control_destroy(service);
                                                ...
                                                void *event_info)
{
	...
	if (POPUP_RESPONSE_OK == response_type) {
		...
		if (ret != APP_CONTROL_ERROR_NONE) {
			...
			ret = app_control_add_extra_data(service, "KEY", "AUTO_OFF");
			SETTING_TRACE_DEBUG("add extra data (KEY,AUTO_OFF) to service ret=%d", ret);

			ret = app_control_send_launch_request(service, security_update_off_service_reply_cb, (void *)data);
			SETTING_TRACE_DEBUG("send launch service (KEY,AUTO_OFF) request ret=%d", ret);
			if (ret != APP_CONTROL_ERROR_NONE) {
				SETTING_TRACE_ERROR("Cannot launch app");
			}
			/*if (ad->data_security_wifi_update_chk) { */
                                              ...
                                              void *event_info)
{
	...
	if (POPUP_RESPONSE_OK == response_type) {
		...
		if (ret != APP_CONTROL_ERROR_NONE) {
			...
			app_control_set_app_id(service, "org.tizen.msa");
			app_control_set_operation(service, "http://org.tizen.msa/appcontrol/operation/query");
			ret = app_control_add_extra_data(service, "KEY", "MANUAL_UPDATE");
			SETTING_TRACE_DEBUG("add extra data to service ret=%d", ret);
			ret = app_control_send_launch_request(service, NULL, NULL);
			SETTING_TRACE_DEBUG("send launch service request ret=%d", ret);
			if (ret != APP_CONTROL_ERROR_NONE) {
				evas_object_del(ad->pop_manual_update);
				ad->pop_manual_update = NULL;
				SETTING_TRACE_ERROR("Cannot launch app");
		...
		/*box */
		Evas_Object *blank = setting_create_blank_rect_customize(ad->pop_progress, 10, 30);
                                                 ...
                                                 void *event_info)
{
	...
	if (!safeStrCmp(_(AUTO_UPDATE), list_item->keyStr)) {
		...
		if (ret != APP_CONTROL_ERROR_NONE) {
			...
			if (chk_status == 1)
				ret = app_control_send_launch_request(service, security_wifi_update_on_service_reply_cb, list_item);
			else
				ret = app_control_send_launch_request(service, security_wifi_update_off_service_reply_cb, list_item);
			SETTING_TRACE_DEBUG("send launch service (MSA_QUERY, %s) request ret=%d", type, ret);
			if (ret != APP_CONTROL_ERROR_NONE) {
				SETTING_TRACE_ERROR("Cannot launch app");
			}
		}
...
static void __setting_security_update_toggle_automatic_chk(void *data, int chk_status, char *keyStr)
{
	SETTING_TRACE_BEGIN;
	SettingSecurityUG *ad = (SettingSecurityUG *) data;
...
static void setting_security_update_mouse_up_Gendial_chk_cb(void *data, Evas_Object *obj, void *event_info)
{
	...
	if (!safeStrCmp(_(AUTO_UPDATE), list_item->keyStr)) {
		__setting_security_update_toggle_automatic_chk(ad, list_item->chk_status, list_item->keyStr);
		...
		app_control_h service = NULL;
		int chk_status = list_item->chk_status;
		/*setting_update_gl_item_chk_status(ad->data_security_wifi_update_chk, chk_status); */
		ret = app_control_create(&service);
		if (ret != APP_CONTROL_ERROR_NONE) {
			SETTING_TRACE_DEBUG("service_create fail!! ret=%d", ret);
			SETTING_TRACE_ERROR("Cannot launch app");
			...
			if (chk_status == 1)
				...
				ret = app_control_send_launch_request(service, security_wifi_update_off_service_reply_cb, list_item);
			SETTING_TRACE_DEBUG("send launch service (KEY, %s) request ret=%d", type, ret);
			if (ret != APP_CONTROL_ERROR_NONE) {
				SETTING_TRACE_ERROR("Cannot launch app");
			}
		}
