...
static void _rot_changed_cb(void *data, Evas_Object *obj, void *event_info)
{
	...
	SETTING_TRACE_DEBUG("diff:%d", elm_win_rotation_get(ad->md.win_main)
			- ad->current_rotation);
		...
		void *data)
{
	...
	elm_object_item_part_text_set(ad->md.navibar_main_it, "elm.text.title",
			_("IDS_ST_OPT_SETTINGS"));
	/*setting_navi_items_update(ad->md.win_main); */
...
static bool setting_main_app_create(void *data)
{
	...
	SETTING_TRACE_DEBUG(
			"[TIME] 3. it took %d msec from main to setting_main_app_create ",
	...
	if (Cfg_Error_Type_Sucess != cfg_operation_ret) {
		...
		default: {
			notifyStr = _(
					"Invalid Result, try to startup the "
					"app again");
			break;
		}
		}
