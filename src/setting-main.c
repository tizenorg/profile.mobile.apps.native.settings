...
static void ___title_toolbar_show(void *data, Evas_Object *obj,
		void *event_info)
{
	SETTING_TRACE_BEGIN;
	setting_main_appdata *ad = (setting_main_appdata *)data;
	...
	if (ad->isInUGMode && ad->ug) {
		...
		if (layout) {
			navi_bar = elm_object_part_content_get(layout,
					"elm.swallow.content");
		}
	} else {
	...
	if (0 == safeStrCmp("SEARCH", support)) {
		...
		if (top_item) {
			elm_object_item_signal_emit(top_item,
					"elm,state,sip,shown", "");
		}
	}
}
...
static void ___title_toolbar_hide(void *data, Evas_Object *obj,
		void *event_info)
{
	SETTING_TRACE_BEGIN;
	setting_main_appdata *ad = (setting_main_appdata *)data;
	...
	if (0 == safeStrCmp("SEARCH", support)) {
		...
		if (top_item) {
			elm_object_item_signal_emit(top_item,
					"elm,state,sip,hidden", "");
		}
	}
}

Evas_Object *setting_main_layout_conform_create(Evas_Object *win_layout,
		Evas_Object *win_obj, void *data)
{
...
static int test(void* data)
{
	...
	SETTING_TRACE("--------------------------------------------");
	char* lang = get_language_list_version_get("TEST-langlist-aplugin");
	SETTING_TRACE("--> LANGUAGE GET %s ", lang);
	FREE(lang);
	SETTING_TRACE("--------------------------------------------");
	get_language_list_version_list("locale", tcf_iterator_langlist, ad);
	SETTING_TRACE("--------------------------------------------");
