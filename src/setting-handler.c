...
void settinig_drawer_hash_init(void *cb)
{
	...
	ad->md.main_list_hash = eina_hash_string_superfast_new(__list_hash_free_cb);
								   ...
								   struct ug_cbs *cbs)
{
	...
	setting_main_appdata *ad = (setting_main_appdata *) data;
	if (!ug_to_load) {
		setting_create_popup(ad, ad->md.win_main, NULL, NO_UG_FOUND_MSG, NULL, 0, false, false, 0);
		return;
	}
