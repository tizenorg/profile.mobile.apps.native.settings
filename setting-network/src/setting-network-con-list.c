		...
		void *event_info)
{
	...
	/*------------------------------------------------------------------- */
	elm_ctxpopup_item_append(ctxpopup, _("IDS_ST_BODY_MYTHEME_CREATE"),
			NULL, setting_network_con_list_click_softkey_create_cb,
			ad);
	elm_ctxpopup_item_append(ctxpopup, _("IDS_ST_BODY_DELETE"), NULL,
			setting_network_con_list_click_softkey_delete_cb, ad);
	/*------------------------------------------------------------------- */
...
/*TODO: select proper connection[] */
	...
	SETTING_TRACE("Default profile:%p, def_apn[%s], def_name[%s], def_id[%s]",
			def_profile, def_apn, def_name, def_id);
	...
	EINA_LIST_FOREACH(ad->profile_list, elist, profile_h)
	{
		...
		item_data->stop_change_cb =
				setting_network_con_list_go_button_cb;
		...
		if (idx == 0) {
			/*defaulty,it the first item */
			ad->selected_profile = item_data;
		}

		item_data->item = elm_genlist_item_append(scroller,
				&(itc_2text_2icon_3), item_data, NULL,
				ELM_GENLIST_ITEM_NONE,
				setting_network_con_list_item_Gendial_mouse_up_cb,
				ad);
		...
		if (!safeStrCmp(def_apn, apn) && !safeStrCmp(def_name, name)
			...
			SETTING_TRACE("Matched [idx:%d, apn:%s, id:%s]!", idx,
					apn, id);
			if (srvType == CONNECTION_CELLULAR_SERVICE_TYPE_INTERNET) {
				...
				if (ad->internet_conn) {
					...
					ad->internet_conn->sub_desc = strdup(
							name);
					elm_object_item_data_set(
							ad->internet_conn->item,
							ad->internet_conn);
	...
	if (srvType == CONNECTION_CELLULAR_SERVICE_TYPE_MMS) {
		if (idx > 0)
			elm_radio_value_set(radio_group, 0);/*the first one */
		if (ad->selected_profile && !isEmptyStr(
				ad->selected_profile->keyStr) && ad->mms_conn) {
			SETTING_TRACE("ad->selected_profile->keyStr:%s",
					ad->selected_profile->keyStr);
			G_FREE(ad->mms_conn->sub_desc);
			FREE(ad->apn_MMS);
			ad->apn_MMS = strdup(ad->selected_profile->keyStr);
			ad->mms_conn->sub_desc = strdup(
					ad->selected_profile->keyStr);
			elm_genlist_item_fields_update(ad->mms_conn->item, "*",
...
static int setting_network_con_list_create(void *cb)
{
	...
	/* [UI] MMS connection */
	ad->navi_it_con_list = setting_push_layout_navi_bar(_(title),
	NULL, /* ARROW STYLE */
	_("IDS_ST_BODY_DELETE"), _("IDS_ST_BODY_MYTHEME_CREATE"),
 ...
 *
 ***************************************************/
static void setting_network_con_list_go_button_cb(void *data, Evas_Object *obj,
		void *event_info)
{
	SETTING_TRACE_BEGIN;
...
static void ___network_con_set_default_cb(connection_error_e result, void *data)
{
	...
	if (srvType == CONNECTION_CELLULAR_SERVICE_TYPE_INTERNET
		...
		elm_object_item_data_set(ad->internet_conn->item,
				ad->internet_conn);
...
/*TODO: select proper connection[] */
	...
	while (connection_profile_iterator_has_next(profile_iter)) {
		...
		SETTING_TRACE("Record[%d] profile is %s(%s)", cnt, apn,
				profile_id);
...
/*TODO: select proper connection[] */
				...
				if (ad->network_ug_pop[0]) {
					evas_object_del(ad->network_ug_pop[0]);
					ad->network_ug_pop[0] = NULL;
				}
...
static void setting_network_con_list_item_Gendial_mouse_up_cb(void *data,
...
static void setting_network_con_list_click_softkey_delete_cb(void *data,
		Evas_Object *obj, void *event_info)
{
	SETTING_TRACE_BEGIN;
	/* error check */
...
static void setting_network_con_list_click_softkey_create_cb(void *data,
		Evas_Object *obj, void *event_info)
{
	SETTING_TRACE_BEGIN;
	/* error check */
...
static Eina_Bool setting_network_con_list_click_softkey_cancel_cb(void *data,
		Elm_Object_Item *it)
{
	SETTING_TRACE_BEGIN;
	/* error check */
