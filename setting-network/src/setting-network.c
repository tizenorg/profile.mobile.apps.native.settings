...
{
	setting_view_update(&setting_view_network_main, data);
}
...
static setting_view *__get_network_view_to_load(void *data)
{
	...
	if (0 == safeStrCmp(ad->view_type_string, "netsearch")) {
...
static void on_app_control(app_control_h service, void *priv)
{
	...
	/*fix load network module repeatedly issue */
