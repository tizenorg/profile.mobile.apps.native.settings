...
/*TODO: select proper connection[] */
		...
		if (rv != CONNECTION_ERROR_NONE) {
			SETTING_TRACE_ERROR("Fail to get profile iterator [%d]",
					rv);
			return NULL;
		}
		...
		while (connection_profile_iterator_has_next(profile_iter)) {
			...
			if (connection_profile_is_cellular_hidden(profile_h,
					&profile_bhidden)
					!= CONNECTION_ERROR_NONE) {
				SETTING_TRACE_ERROR(
						"Fail to get profile hidden attr (true/false)");
				continue;
			}

			if (connection_profile_get_cellular_apn(profile_h, &apn)
					!= CONNECTION_ERROR_NONE) {
			...
			if (profile_bhidden == false
				...
				SETTING_TRACE("profile hidden attribute : %d ",
						profile_bhidden);
			}
			G_FREE(apn);
			G_FREE(profile_id);
...
static void setting_network_ug_cb_resize(void *data, Evas *e, Evas_Object *obj,
		void *event_info)
{
	SettingNetworkUG *ad = (SettingNetworkUG *)data;
		...
		app_control_h service, void *priv)
{
	 ...
	 * &(networkUG->whitelist_doc),&(networkUG->whitelist_root_node)); */
		...
		void *priv)
{
	...
	for (; i < SIM_CARDS_MAX; i++) {
		...
		setting_network_popup_delete(networkUG, i);
	}
...
void setting_network_popup_delete(void *data, int sim_ix)
{
	...
	if (ad->network_ug_pop[sim_ix] != NULL) {
		evas_object_del(ad->network_ug_pop[sim_ix]);
		ad->network_ug_pop[sim_ix] = NULL;
	}
	if (ad->popup != NULL) {
		evas_object_del(ad->popup);
		ad->popup = NULL;
		...
		void *user_data)
{
	...
	if (result == TAPI_NETWORK_NO_ERR) {
		...
		/*?????????*/
		//vconf_set_int(VCONFKEY_SETAPPL_NETWORK_MODE_INT, ad->net_mode[sim_ix]);
		...
		void *user_data)
{
	...
	sim_ix = get_sim_ix_from_tapi_handle(handle, ad);
		if (sim_ix < 0)
			return;
	setting_network_popup_delete(ad, sim_ix);
...
void ___sort_merge_handle(void *usrdata, TelNetworkPlmnList_t *list)
{
	...
	for (idx = 0; idx < tempcount; idx++) {
		...
		for (j = 0; j < cnt; j++) {
			if (0 == safeStrCmp(
					...
					|| 0 == safeStrCmp(
					ad->plmn_info.network_list[j].plmn,
					list->network_list[idx].plmn)) {
				alread_exist = TRUE;
				break;
			}
	...
	SETTING_TRACE("cnt:%d, list->networks_count=%d", cnt,
			list->networks_count);
	SETTING_TRACE("*****************************************************");
		...
		void *user_data)
{
	...
	if (result == TAPI_NETWORK_NO_ERR) {
...
#endif
#if 1
			if (TAPI_NETWORK_SELECTIONMODE_AUTOMATIC != ad->sel_net && 0 == safeStrCmp(
					pa_net_name,
					ad->plmn_info.network_list[cnt].network_name) && ad->plmn_info.network_list[cnt].access_technology == ad->sel_act) {
				/* [ UI UPDATE ]*/
				/*let select nothing */
		...
		SETTING_TRACE("Found %d valid networks in total[%d]", valid_cnt,
				cnt);
...
/*TODO: select proper handle:*/
	if (0 == safeStrCmp(list_item->keyStr,
		...
		if (tapi_ret != TAPI_API_SUCCESS) {
			SETTING_TRACE_DEBUG(
					"*** [ERR] tel_select_network_automatic." " tapi_ret=%d ***",
					tapi_ret);
		} else {
		...
		for (; cnt < ad->plmn_info.networks_count; cnt++) {
			SETTING_TRACE(
					"ad->plmn_info.network_list[cnt].network_name:%s",
					ad->plmn_info.network_list[cnt].network_name);
			memset(name, 0, MAX_COMMON_BUFFER_LEN);
			snprintf(name, MAX_COMMON_BUFFER_LEN, "%s",
					ad->plmn_info.network_list[cnt].network_name);
...
Evas_Object *__create_searching_popup(void *data)
{
	...
	elm_popup_align_set(popup, ELM_NOTIFY_ALIGN_FILL, 1.0);
	eext_object_event_callback_add(popup, EEXT_CALLBACK_BACK,
			__search_network_cancel_cb, list_item);
	evas_object_size_hint_weight_set(popup, EVAS_HINT_EXPAND,
	EVAS_HINT_EXPAND);
		...
		void *event_info)
{
	...
	if (0 == safeStrCmp(list_item->keyStr,
		...
		SETTING_TRACE("[Radio index:%d] Radio checked",
				list_item->chk_status);
...
/*TODO: select proper connection[] */
	...
	while (connection_profile_iterator_has_next(profile_iter)) {
		...
		cnt++;
		if (CONNECTION_PROFILE_TYPE_CELLULAR != profile_type
				|| ad->profile_service_type != service_type) {
			continue;
		}
		ad->profile_list = eina_list_sorted_insert(ad->profile_list,
				__profile_item_sort_cb, profile_h);
...
static Setting_Cfg_Node_T s_cfg_node_array[] = { {
	...
	NULL,
	"viewtype:frontpage;tab:first;keyword:IDS_ST_MBODY_MOBILE_DATA",
	0,
	0,
	0,
	Cfg_Item_View_Node,
	...
	NULL,
	"viewtype:frontpage;tab:first;keyword:IDS_ST_BODY_DATA_ROAMING",
	0,
	0,
	0,
	Cfg_Item_View_Node,
	...
	NULL,
	"viewtype:frontpage;tab:first;keyword:IDS_ST_BODY_NETWORK_MODE",
	0,
	0,
	0,
	Cfg_Item_View_Node,
	...
	NULL,
	NULL }, {
	"IDS_ST_BODY_CONNECTIONS",
	NULL,
	"viewtype:frontpage;tab:first;keyword:IDS_ST_BODY_CONNECTIONS",
	0,
	0,
	0,
	Cfg_Item_View_Node,
...
int setting_network_get_state_mobile_data(int *value)
{
	...
	int ret = system_settings_get_value_bool(
			SYSTEM_SETTINGS_KEY_3G_DATA_NETWORK_ENABLED, &val);
