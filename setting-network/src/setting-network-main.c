...
#define MOBILE_DATA_WITHOUT_WIFI_STR \
	...
	"ITHOUT_WI_FI_CONNECTION"
#define UMTS_MODE_WORK_OUTSIDE_STR \
...
#define __switch_radio_state(cur_data, dst_state, dst_key) do { \
		if (cur_data) {\
			...
			elm_genlist_item_fields_update(cur_data->item, "*", \
...
void __network_sub_list_sel_cb(void *data, Evas_Object *obj, void *event_info)
{
	...
	sim_ix = get_sim_ix_from_tapi_handle(data_subItem->modem_handle, ad);
	if (sim_ix < 0)
		return;

	SETTING_TRACE("ad->net_mode[%d]=%d", sim_ix, ad->net_mode[sim_ix]);
	SETTING_TRACE("elm_radio_value_get(ad->data_net_mode[%d]->rgd):%d",
			sim_ix, elm_radio_value_get(ad->data_net_mode[sim_ix]->rgd));
	SETTING_TRACE("data_subItem->chk_status=%d", data_subItem->chk_status);

	if (ad->net_mode[sim_ix] == data_subItem->chk_status) {
	...
	ad->network_ug_pop[sim_ix] = setting_create_popup_with_progressbar(ad,
			ad->win_get, PROGRESSBAR_STYLE,
			NULL, "IDS_ST_BUTTON2_PROCESSING_ING",
			NULL, 0.0, TRUE, FALSE, 0);

	elm_radio_value_set(ad->data_net_mode[sim_ix]->rgd, data_subItem->chk_status);
	ad->net_mode[sim_ix] = data_subItem->chk_status;
	...
	if (tapi_ret != TAPI_API_SUCCESS) {
		...
		ret = setting_get_int_slp_key(
				ad->data_net_mode[sim_ix]->int_slp_setting_binded,
				&(ad->net_mode[sim_ix]), &err);
		if (ret == SETTING_RETURN_FAIL)
			SETTING_TRACE_ERROR("failed to get vconf");
	} else {
	...
	if (ad->network_mode_popup[sim_ix]) {
		evas_object_del(ad->network_mode_popup[sim_ix]);
		ad->network_mode_popup[sim_ix] = NULL;
	}
}
...
/**
 * Vconf callback for network restriction mode (
...
static char *_get_network_mode_sub_desc()
{
	...
	if (-1 != k) {
		...
		SETTING_TRACE("VCONFKEY_SETAPPL_NETWORK_MODE_INT value:%d on success ",
				value);
	}
	return sub_des;
}
...
static void setting_network_mode_popup(void *data, int sim_ix)
{
	...
	ad->network_mode_popup[sim_ix] = setting_create_popup_with_list(&genlist, ad,
			ad->win_get, _("IDS_ST_BODY_NETWORK_MODE"), NULL, 0,
			false, false, 0);
	_P(ad->network_mode_popup[sim_ix]);
	/*Evas_Object *parentItem = ad->network_mode_popup[sim_ix]; */
	...
	if (is_lte_on_feature(ad)) {
		...
		ad->lte_wcdma_gsm[sim_ix] = setting_create_Gendial_field_1radio(
		...
		ad->lte_wcdma_gsm[sim_ix]->modem_handle = ad->handle[sim_ix];
	}
	...
	ad->wcdma_gsm[sim_ix] = setting_create_Gendial_field_1radio(genlist,
			...
			pszStrId, __network_default_rd_change);
	ad->wcdma_gsm[sim_ix]->modem_handle = ad->handle[sim_ix];
	...
	ad->wcdma_only[sim_ix] = setting_create_Gendial_field_1radio(genlist,
			&itc_multiline_1text_1icon, __network_sub_list_sel_cb,
			ad, SWALLOW_Type_1RADIO_RIGHT, rdg,
			TAPI_NETWORK_MODE_WCDMA, /* chk_status */
			pszStrId, __network_default_rd_change);
	ad->wcdma_only[sim_ix]->modem_handle = ad->handle[sim_ix];
	...
	ad->gsm_only[sim_ix] = setting_create_Gendial_field_1radio(genlist,
			&itc_multiline_1text_1icon, __network_sub_list_sel_cb,
			ad, SWALLOW_Type_1RADIO_RIGHT, rdg,
			TAPI_NETWORK_MODE_GSM, /* chk_status */
			pszStrId, __network_default_rd_change);
	ad->gsm_only[sim_ix]->modem_handle = ad->handle[sim_ix];


/*TODO: this part should be changed (use tapi instead):*/
	...
	do {
		if (is_lte_on_feature(ad)) {
			if ((value & TAPI_NETWORK_MODE_LTE)
				...
				if (ad->lte_wcdma_gsm[sim_ix]) {
					elm_radio_value_set(
							ad->data_net_mode[sim_ix]->rgd,
							ad->lte_wcdma_gsm[sim_ix]->chk_status);
					SETTING_TRACE("ad->lte_wcdma_gsm[%d]->chk_status=%d checked",
							sim_ix, ad->lte_wcdma_gsm[sim_ix]->chk_status);
					break;
				} else {
					SETTING_TRACE_ERROR("ad->lte_wcdma_gsm[%d] is NULL", sim_ix);
				}
			}
		}
		...
		if ((value & TAPI_NETWORK_MODE_WCDMA)
			...
			if (ad->wcdma_gsm[sim_ix]) {
				elm_radio_value_set(ad->data_net_mode[sim_ix]->rgd,
						ad->wcdma_gsm[sim_ix]->chk_status);
				SETTING_TRACE("ad->wcdma_gsm[%d]->chk_status=%d checked",
						sim_ix, ad->wcdma_gsm[sim_ix]->chk_status);
			} else {
				SETTING_TRACE_ERROR("ad->wcdma_gsm[%d] is NULL", sim_ix);
			}
		} else if (value & TAPI_NETWORK_MODE_WCDMA) {
			SETTING_TRACE("TAPI_NETWORK_MODE_WCDMA selected");

			if (ad->wcdma_only[sim_ix]) {
				elm_radio_value_set(ad->data_net_mode[sim_ix]->rgd,
						ad->wcdma_only[sim_ix]->chk_status);
				SETTING_TRACE("ad->wcdma_only[%d]->chk_status=%d checked",
						sim_ix, ad->wcdma_only[sim_ix]->chk_status);
			} else {
				SETTING_TRACE_ERROR("ad->wcdma_only[%d] is NULL", sim_ix);
			}
		} else if (value & TAPI_NETWORK_MODE_GSM) {
			SETTING_TRACE("TAPI_NETWORK_MODE_GSM selected");

			if (ad->gsm_only[sim_ix]) {
				elm_radio_value_set(ad->data_net_mode[sim_ix]->rgd,
						ad->gsm_only[sim_ix]->chk_status);
				SETTING_TRACE("ad->gsm_only[%d]->chk_status=%d checked",
						sim_ix, ad->gsm_only[sim_ix]->chk_status);
			} else {
				SETTING_TRACE_ERROR("ad->gsm_only[%d] is NULL", sim_ix);
			}
		} else {
...
//_get_network_selected_desc(ad, sel_network_desc, MAX_COMMON_BUFFER_LEN);
	...
	for (sim_ix = 0; sim_ix < SIM_CARDS_MAX; sim_ix++) {
		if (ad->handle[sim_ix]) {
			...
			free(spn_name);
			ad->data_net_mode[sim_ix] = setting_create_Gendial_field_def(genlist,
						&itc_2text_3_parent,
						setting_network_main_item_Gendial_mouse_up_cb, ad,
				...
				if (ad->data_net_mode[sim_ix]) {
					ad->data_net_mode[sim_ix]->modem_handle = ad->handle[sim_ix];
					ad->data_net_mode[sim_ix]->int_slp_setting_binded =
							INT_SLP_SETTING_NETWORK_MODE;
					__BACK_POINTER_SET(ad->data_net_mode[sim_ix]);
				} else {
					SETTING_TRACE_ERROR("ad->data_net_mode[%d] is NULL", sim_ix);
				}
				tel_get_network_mode(ad->handle[sim_ix], setting_tapi_get_band_cb, ad);
		}
	}
	...
	vconf_notify_key_changed(VCONFKEY_SETAPPL_STATE_DATA_ROAMING_BOOL,
			__data_roaming_change_cb, ad);
	vconf_notify_key_changed(VCONFKEY_SETAPPL_NETWORK_RESTRICT_MODE,
			__net_restriction_mode_vconf_change_cb, ad);
		...
		void *event_info)
{
	...
	if (POPUP_RESPONSE_OK == response_type) {
		/*changing of USE_PACKET_DATA was trigged by betwork
		 * searching */
		...
		ad->timer = ecore_timer_add(TIME_TO_TURN_OFF_3G,
				__change_search_view_on_timer, ad);
		...
		Evas_Object *obj, void *event_info)
{
	...
	if (safeStrStr(_(list_item->keyStr), _("IDS_ST_BODY_SUFIX_NETWORK_MODE")) != NULL) {
		...
		sim_ix = get_sim_ix_from_tapi_handle(list_item->modem_handle, ad);
		...
		void *event_info)
{
	...
	SettingNetworkUG *ad = (SettingNetworkUG *)data;
	int response_type = btn_type(obj);
	SETTING_TRACE("elm_object_parent_widget_get(obj):%p",
