...
static void __setting_network_con_genlist_create(void *data)
{
	...
	/*item = elm_genlist_item_append(ad->connections_gl, &itc_seperator,
	 NULL, NULL,
	...
	Setting_GenGroupItem_Data *mms_conn = setting_create_Gendial_field_def(
			...
			setting_network_con_item_Gendial_mouse_up_cb, ad,
			SWALLOW_Type_INVALID, NULL, NULL, 0,
			STR_SETTING_MMS_CONNECTIONS, g_strdup(ad->apn_MMS),
			NULL);
	...
	/* add other APN
	 ...
	 ad->add_apn_timer = ecore_timer_add(0.5,
	 (Ecore_Task_Cb) __add_other_apn, ad);
	 } else {
	 __add_other_apn(ad);
	 }
	 */
...
static void ctxpopup_item_select_cb(void *data, Evas_Object *obj,
		void *event_info)
{
	SETTING_TRACE_BEGIN;
	/* error check */
		...
		void *event_info)
{
	...
	elm_ctxpopup_item_append(ctxpopup, _(KeyStr_Reset_Def), NULL,
			ctxpopup_item_select_cb, ad);
...
static void setting_network_con_item_Gendial_mouse_up_cb(void *data,
...
#endif
	else if (!safeStrCmp("IDS_ST_BODY_INTERNET_CONNECTION",
			list_item->keyStr)) {/*connections.. */

		FREE(ad->con_name);
		SETTING_TRACE("list_item->keyStr:%s, ad->con_name:%s",
				list_item->keyStr, ad->con_name);
...
static void setting_network_con_click_softkey_reset_cb(void *data,
		Evas_Object *obj, void *event_info)
{
	SETTING_TRACE_BEGIN;
	/* error check */
...
void _reset_do_default_cb(connection_error_e result, void *user_data)
{
	...
	SettingNetwork *ad = (SettingNetwork *)user_data;
...
static void setting_network_con_reset_popup_cb(void *data, Evas_Object *obj,
		void *event_info)
{
	SETTING_TRACE_BEGIN;
...
static Eina_Bool setting_network_con_click_softkey_cancel_cb(void *data,
		Elm_Object_Item *it)
{
	SETTING_TRACE_BEGIN;
