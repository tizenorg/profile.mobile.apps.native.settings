...
static bool __package_info_privilege_info_cb(const char *privilege_name, void *user_data)
{
	int ret = PRVINFO_ERROR_NONE;
	char *dspl_nam = NULL;

	ret = privilege_info_get_display_name("3.0", privilege_name, &dspl_nam);

	ret = privilege_info_get_description("3.0", privilege_name, &dspl_nam);
