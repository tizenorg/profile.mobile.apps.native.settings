...
static void _gl_Gendial_Edit_sel(void *data, Evas_Object *obj, void *event_info)
{
	SETTING_TRACE_BEGIN
	;
	retm_if(event_info == NULL, "Invalid argument: event info is NULL");
	...
	SETTING_TRACE_ERROR("Show input panel data=%p e_i=%p", data,
			item_data->win_main);
//	__add_entry_without_layout(item_data, item_data->win_main);
	Evas_Object *entry = __add_entry_without_layout(item_data, obj);
	elm_object_part_content_set(obj, "elm.swallow.content", entry);
//	__add_entry_padding(item_data, obj);
}
...
#if 0
	SETTING_TRACE("part:%s -------- item_data->itc : %s ",
			part, item_data->itc);
#endif
	char *ret_str = NULL;
	/*------------------------------------------------------------- */
		...
		const char *part)
{
	...
	if (!safeStrCmp(part, "elm.swallow.icon")) { /* LEFT AREA */
		...
		if (item_data->swallow_type
				== SWALLOW_Type_LAYOUT_EDITFIELD&& item_data->isPasswordFlag == TRUE)
			return NULL;
		...
		char *accepted, char *rejected)
{
	...
	item_data->win_main = genlist; // elm_object_parent_widget_get(genlist);
	...
	if (gl_sel) {
		...
		item_data->item = elm_genlist_item_append(genlist, itc,
				item_data, NULL, ELM_GENLIST_ITEM_NONE,
				_gl_Gendial_Edit_sel, item_data);
	}
	elm_genlist_item_select_mode_set(item_data->item,
			ELM_OBJECT_SELECT_MODE_MAX); //DISPLAY_ONLY);
