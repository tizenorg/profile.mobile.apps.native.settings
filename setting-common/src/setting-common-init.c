...
static Evas_Object *_layout_conform_create(Evas_Object *win_obj, MainData *ad)
{
	SETTING_TRACE_BEGIN;
	...
	ad->conform = elm_conformant_add(win_obj);
	if (!ad->conform)
		return NULL;
	...
	evas_object_size_hint_weight_set(ad->conform, EVAS_HINT_EXPAND,
			EVAS_HINT_EXPAND);
	evas_object_size_hint_align_set(ad->conform, EVAS_HINT_FILL,
			EVAS_HINT_FILL);
	elm_win_resize_object_add(win_obj, ad->conform);
	...
	/* Indicator bg */
	indicator_bg = elm_bg_add(ad->conform);
	elm_object_style_set(indicator_bg, "indicator/headerbg");
	elm_object_part_content_set(ad->conform, "elm.swallow.indicator_bg",
			indicator_bg);
	evas_object_show(indicator_bg);

	evas_object_data_set(win_obj, "conformant", ad->conform);
	return layout;
}
...
int view_init(MainData *ad, char *name)
{
	...
	ad->ly_main = _layout_conform_create(ad->win_main, ad);
	...
	ad->navibar_main_it = elm_naviframe_item_push(ad->navibar_main, name,
			NULL, NULL, ad->view_layout, NULL);
	elm_naviframe_item_title_enabled_set(ad->navibar_main_it, EINA_TRUE,
			EINA_TRUE);
	elm_object_item_domain_text_translatable_set(ad->navibar_main_it,
