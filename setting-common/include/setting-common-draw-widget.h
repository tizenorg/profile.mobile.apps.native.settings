 ...
 */
extern void setting_mouse_up_Gendial_list_radio_cb(void *data, Evas_Object *obj,
		void *event_info);
 ...
 */
extern Evas_Object *setting_create_slider(Evas_Object *win_main,
		Setting_GenGroupItem_Data *item_data);
 ...
 * @return a slider container object
 */
extern Evas_Object *setting_create_5step_slider(Evas_Object *win_main,
...
extern void create_image_box_add_ex(void *data, Evas_Object *win_main,
		Evas *evas, char *img_path, int img_w, int img_h,
		Evas_Object_Event_Cb mouse_down_cb,
		Evas_Object_Event_Cb mouse_up_cb, Evas_Object **image_box,
		Evas_Object **img);
 ...
 * @return a lable
 */
extern Evas_Object *setting_create_lable(Evas_Object *parent, const char *text,
 ...
 * @return an object to show an image
 */
extern Evas_Object *setting_create_image_with_round_corner(Evas *evas,
		const char *img_path, int w, int h);
 ...
 */
extern Evas_Object *setting_create_editfiled(Evas_Object *win_main, char *title,
		char *def_str, char *guide_text);
/**
...
extern Evas_Object *setting_create_button(Evas_Object *parent,
		const char *btn_str, const char *btn_style, void *btn_click_cb,
		void *cb_data);

extern void setting_nf_check_del_cb(void *data, Evas *e, Evas_Object *obj,
		void *event_info);
extern void setting_nf_check_change_cb(void *data, Evas_Object *obj,
		void *event_info);
...
#define ADD_NAVIFRAME_CHECK(parent, navi_it, vconf) \
	{\
		...
		elm_object_item_part_content_set(navi_it, "title_right_btn", \
				check);\
		(void)vconf_notify_key_changed(vconf, \
				setting_nf_check_vconf_cb, check);\
		evas_object_smart_callback_add(check, "changed", \
				setting_nf_check_change_cb, vconf);\
		evas_object_event_callback_add(check, EVAS_CALLBACK_DEL, \
				setting_nf_check_del_cb, vconf);\
	}
 ...
 */
extern void
setting_create_navi_bar_buttons(char *title_str, char *lbutton_str,
		setting_call_back_func lbutton_click_cb, void *cb_data,
		Evas_Object *eo_view, Evas_Object *navigate_bar,
...
extern Evas_Object *setting_create_layout_navi_bar_scroller(
		Evas_Object *win_layout, Evas_Object *win_obj, char *title_str,
...
extern Evas_Object *setting_create_layout_navi_bar_genlist(
		Evas_Object *win_layout, Evas_Object *win_obj, char *title_str,
		char *lbutton_str, char *rbutton_str, void *lbutton_click_cb,
 ...
 * specialized navi_bar, which contents at most 3 buttons
 */
extern Elm_Object_Item *setting_push_layout_navi_bar(char *title_str,
		char *lbutton_str, char *rbutton_str, char *mbutton_str,
		void *lbutton_click_cb, void *rbutton_click_cb,
		void *mbutton_click_cb, void *cb_data, Evas_Object *eo_view,
		Evas_Object *navi_bar, Evas_Object **titleobj);

/**
 * The API to push a scroller object(@param[in][scroller]) to the specialized
 * navi_bar, which contents at most 2 buttons
 */
extern Elm_Object_Item *setting_push_layout_navi_bar_scroller(
		Evas_Object *win_main, char *title_str, char *lbutton_str,
...
/**
 * To create an icon which looks clickable(if pressed, it is dim color;if
 * unpressed, it is normal color)
 *
 * @param[in] parent
 * @param[in] img_path
 * @param[in] up_cb
 ...
 * @param[in] data
 */
extern Evas_Object *setting_create_icon(Evas_Object *parent,
		const char *img_path, Evas_Object_Event_Cb up_cb,
		Evas_Object_Event_Cb down_cb, Evas_Object_Event_Cb move_cb,
		void *data);
/**
 * to make sbu-obj the same size as parent-obj.Notice: subobj is the only
 * sub-obj @parent wants to display
 *
 * @param[in] parent
 * @param[in] subobj
 */
...
extern Evas_Object *setting_create_image_button(Evas_Object *parent,
		const char *btn_img, setting_call_back_func clicked_cb,
		setting_call_back_func unpressed_cb, void *data);
...
extern void setting_entry_entry_set(Evas_Object *entry, const char *text,
		const int font_size);
extern void setting_update_gl_item_chk_status(
		Setting_GenGroupItem_Data *item_data, int status);
extern void setting_force_update_gl_item_chk_status(
		Setting_GenGroupItem_Data *item_data, int status);

extern void setting_conformant_keypad_state(Evas_Object *conform, bool enable);

extern Evas_Object *setting_create_navibar_title_text_btn(Evas_Object *parent,
