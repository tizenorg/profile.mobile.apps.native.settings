...
static void setting_accessibility_screen_reader_sound_feedback_vconf_update(int state)
{
	vconf_set_bool(VCONFKEY_SETAPPL_ACCESSIBILITY_SCREEN_READER_SOUND_FEEDBACK, state ? 1 : 0);
}

static void setting_accessibility_screen_reader_sound_feedback_check_update(void *data,
														   Evas_Object *obj, void *event_info)
{
	SETTING_TRACE_BEGIN;
	retm_if(data == NULL, "Data parameter is NULL");
	...
	SETTING_TRACE_DEBUG("check_status: %i", list_item->chk_status);
	setting_accessibility_screen_reader_sound_feedback_vconf_update(
			list_item->chk_status);
	SETTING_TRACE_END;
}


static void setting_accessibility_screen_reader_settings_sound_feedback_mouse_up_Gendial_list_cb(
			void *data, Evas_Object *obj, void *event_info)
{
	SETTING_TRACE_BEGIN;
	Elm_Object_Item *item = (Elm_Object_Item *) event_info;
	...
	elm_genlist_item_selected_set(item, EINA_FALSE);
	setting_update_gl_item_chk_status(list_item, list_item->chk_status ? 0 : 1);
	setting_accessibility_screen_reader_sound_feedback_vconf_update(list_item->chk_status ? 1 : 0);
}
...
// Item 1: Speech Rate
	...
	ad->speech_rate_item = setting_create_Gendial_field_def(
			 ...
			 buf,
			 NULL);
	tts_setting_set_speed_changed_cb(__tts_setting_speed_changed_cb, ad->speech_rate_item);
...
// Item 6 : Sound Feedback
	vconf_ret = vconf_get_bool(VCONFKEY_SETAPPL_ACCESSIBILITY_SCREEN_READER_SOUND_FEEDBACK, &vconf_val);
	if (vconf_ret != 0) {
	LOGE("Failed to get vconf bool value "
				"VCONFKEY_SETAPPL_ACCESSIBILITY_SCREEN_READER_SOUND_FEEDBACK. Error: %d", vconf_ret);
	}

	ad->sound_feedback_item = setting_create_Gendial_field_def(
				genlist, &itc_1text_1icon,
				setting_accessibility_screen_reader_settings_sound_feedback_mouse_up_Gendial_list_cb,
				ad, SWALLOW_Type_1ICON_1RADIO, NULL,
				NULL, vconf_val ? 1 : 0,
				_(DEVOPTION_STR_ACCESSIBILITY_SCREEN_READER_SETTINGS_SOUND_FEEDBACK),
				NULL,
				setting_accessibility_screen_reader_sound_feedback_check_update);
	...
	elm_genlist_item_select_mode_set(ad->multiline_status_bar_information_description->item, ELM_OBJECT_SELECT_MODE_DISPLAY_ONLY);
