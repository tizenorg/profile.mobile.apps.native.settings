...
static void storageUg_misces_sel_all_clicked(void *data, Evas_Object *obj,
		void *event_info);
static inline void storageUg_misces_gl_append_select_all_item(
		SettingStorageUG *ad);
static inline void storageUg_misces_gl_remove_select_all_item(
		SettingStorageUG *ad);
 ...
 */
static void setting_storage_ctx_click_softkey_cb(void *data, Evas_Object *obj,
		void *event_info)
{
	SETTING_TRACE_BEGIN;
	/* error check */
...
static inline Evas_Object *_create_ly_misces_no_item(Evas_Object *parent)
{
	...
	elm_image_file_set(icon,
			SETTING_ICON_PATH_CFG "/00_nocontents_text.png", NULL);
	elm_object_part_content_set(layout, "nocontents.image", icon);

	elm_object_domain_translatable_part_text_set(layout, "elm.text",
			SETTING_PACKAGE,
...
static inline void storageUg_meisces_get_list(SettingStorageUG *ad)
{
	...
	/*
	 snprintf(condition, 1024,
	 "(MEDIA_TYPE=4) and (MEDIA_PATH LIKE \'%s/.tmp_download/%%\'"
	...
	ret = media_info_foreach_media_from_db(filter,
			storageUg_misces_media_item_cb, ad);
	if (ret != MEDIA_CONTENT_ERROR_NONE) {
		media_filter_destroy(filter);
		SETTING_TRACE_ERROR(
...
static void storageUg_misces_handle_selecting(SettingStorageUG *ad)
{
	...
	if (0 < ad->misces_checked) {
		...
		snprintf(text, sizeof(text), _(STORAGEUG_STR_N_SELECTED),
				ad->misces_checked);
		...
		elm_object_item_part_text_set(ad->misces_navi_it, "subtitle",
				"");
		elm_object_item_signal_emit(ad->misces_navi_it,
				"elm,state,subtitle,hide", "elm");
		...
		int checked)
{
	...
	if (checked) {
		...
		ad->misces_checked++;
		ad->misces_sel_list = eina_list_append(ad->misces_sel_list,
				strdup(id));
	} else {
		...
		ad->misces_checked--;
		found = eina_list_search_unsorted_list(ad->misces_sel_list,
				storageUg_compare_str, id);
		free(eina_list_data_get(found));
		ad->misces_sel_list = eina_list_remove_list(ad->misces_sel_list,
				found);
	}
 ...
 */
static inline void setting_storage_select_all_chk_btn_cb(void *data,
		Evas_Object *obj, void *event_info)
{
	...
	SETTING_TRACE_ERROR("2 : ad->misces_is_select_all--->%d",
		...
		SettingStorageUG *ad)
{
	...
	item_data = setting_create_Gendial_field_def(ad->gl_misces,
			&(itc_1text_1icon), storageUg_misces_sel_all_clicked,
			ad, SWALLOW_Type_1ICON_1RADIO, NULL, NULL, checked,
...
static inline void storageUg_misces_gl_append_items(SettingStorageUG *ad)
{
	...
	EINA_LIST_FOREACH(ad->misces_list, cur, node)
	{
		...
		found = eina_list_search_unsorted_list(tmp_sel_list,
				storageUg_compare_str, node->id);
		if (found) {
			checked = 1;
		...
		item_data = setting_create_Gendial_field_def(ad->gl_misces,
				&(itc_1text_1icon),
				storageUg_misces_genlist_sel, ad,
				SWALLOW_Type_1CHECK_RIGHT_INVISIBLE, NULL, NULL,
				checked, node->name, node->path,
...
static void storageUg_misces_delete_list(SettingStorageUG *ad)
{
	...
	/* second */
	Elm_Object_Item *it = elm_genlist_nth_item_get(ad->gl_misces, 1);
	...
	if (0 == ad->misces_sz_all) {
		...
		if (elm_object_item_part_content_get(ad->misces_navi_it,
				"title_right_btn")) {
			Evas_Object *btn_done = elm_object_item_part_content_unset(
					ad->misces_navi_it, "title_right_btn");
		...
		if (elm_object_item_part_content_get(ad->misces_navi_it,
				"title_left_btn")) {
			Evas_Object *btn_cancel = elm_object_item_part_content_unset(
					ad->misces_navi_it, "title_left_btn");
...
void storageUg_misces_delete_files_cb(int fn_result, SettingStorageUG *ad)
{
	...
	if (SETTING_RETURN_SUCCESS != fn_result) {
		SETTING_TRACE_ERROR("storageUg_misces_delete_files() Fail(%d)",
				fn_result);
		...
		void *event_info)
{
	...
	if (ad->misces_checked) {
		...
		ad->del_worker = storageUg_start_async_worker(
 ...
 */
static void storageUg_misces_cancel_cb(void *data, Evas_Object *obj,
		void *event_info)
{
	SETTING_TRACE_BEGIN;
...
static int storageUg_misces_create(void *data)
{
	...
	if (ad->misces_sz_all > 0) {
		...
		/* STR_DELETE_ITEMS */
		ad->misces_navi_it = setting_push_layout_navi_bar(
	...
	elm_naviframe_item_pop_cb_set(ad->misces_navi_it,
