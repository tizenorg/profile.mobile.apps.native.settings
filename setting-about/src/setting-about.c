...
bool setting_about_ug_on_create(void *priv)
{
	...
	//elm_win_resize_object_add(aboutUG->win_get, elm_bg_add(aboutUG->win_get));//aboutUG->win_main_layout);
...
//setting_view_node_table_register(&setting_view_about_main, NULL);
	...
	/*  creating a view. */
	setting_create_Gendial_itc("default"/*SETTING_GENLIST_ICON_1LINE_STYLE*/, &(aboutUG->itc_1text));
...
//setting_view_create(&setting_view_about_main, (void *)aboutUG);
	aboutUG->genlsit = elm_genlist_add(aboutUG->win_get);
	elm_win_resize_object_add(aboutUG->win_get, aboutUG->genlsit);
	evas_object_show(aboutUG->genlsit);

	Setting_GenGroupItem_Data *item_data = calloc(1, sizeof(Setting_GenGroupItem_Data));
	item_data->keyStr = "blaaa";
	elm_genlist_item_append(aboutUG->genlsit,
							&aboutUG->itc_1text, item_data,
