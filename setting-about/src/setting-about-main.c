...
static void __soft_back_key_cb(
...
static Eina_Bool __setting_about_popup_show_delay(void *data)
{
	...
	if (NULL == ad->popup) {
		...
		/*ad->popup = setting_create_popup(ad, ad->,
		...
		Evas_Object *obj, void *event_info)
{
	...
	if (!safeStrCmp(_("IDS_ST_HEADER_MANAGE_CERTIFICATES_ABB"),
			list_item->keyStr)) {
		__setting_about_main_certificates_clicked(data);
	} else if (!safeStrCmp(_("IDS_ST_BODY_REGULATORY_INFORMATION_ABB"),
			list_item->keyStr)) {
		__setting_about_popup_regulatory_info(data);
	} else if (!safeStrCmp(_("IDS_ST_MBODY_LEGAL_INFORMATION_ABB"),
			list_item->keyStr)) {
		__setting_about_main_licence_launch(data);
	} else if (!safeStrCmp(_("IDS_ST_MBODY_SOFTWARE_UPDATE"),
			list_item->keyStr)) {
#ifdef SUPPORT_FOTA
		SettingAboutUG *ad = (SettingAboutUG *)list_item->userdata;

		Evas_Object *back_btn = NULL;//elm_object_item_part_content_get(
		...
		Evas_Object *obj, void *event)
{
	...
	if (ad->item_dev_name) {
		Elm_Object_Item *selected_item = NULL;//elm_genlist_at_xy_item_get(
