		...
		void *event_info)
{
	...
	/* Bundle send message */
	app_control_add_extra_data(ad->bundle_data, "result", "rbutton_click");
...
static void _close_popup_ex(void *data)
{
		...
		EINA_LIST_FOREACH(langlist, elist, pnode)
		{
			if (safeStrCmp(ad->selected_locale, pnode->title)
				...
				if (region_automatic == 1) {
					...
					int ret = setting_phone_region_format_set_dateformat(
							pnode->locale, ad);
	...
	if (caller) {
		...
		/* Bundle send message */
		app_control_add_extra_data(ad->bundle_data, "result",
				"rbutton_click");
		...
		ad->pop_progress_lang = setting_create_popup_with_progressbar(
				ad, ad->ly_language,
				PROGRESSBAR_STYLE,
				NULL, KeyStr_Loading,
				_done_popup_resp_cb,
