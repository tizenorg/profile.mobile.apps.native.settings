...
static void _click_softkey_cancel_cb2(void *data, Evas_Object *obj,
		void *event_info)
{
	_click_softkey_cancel_cb(data, NULL);
}
...
static int setting_phone_language_region_create(void *cb)
{
	...
	if (ad->data_region_fmt_example)
		elm_genlist_item_select_mode_set(
				ad->data_region_fmt_example->item,
...
static int setting_phone_language_region_update(void *cb)
{
	...
	if (ad->ly_language != NULL) {
		...
		/*refresh the title */
		/*"IDS_ST_BODY_LANGUAGE_AND_KEYBOARD")); */
		...
		if (ad->data_region_fmt) {
			...
			int ret = vconf_get_bool(
					VCONFKEY_SETAPPL_REGION_AUTOMATIC_BOOL,
					&region_automatic);
			if (ret == VCONF_ERROR) {
				SETTING_TRACE_ERROR(
						"get vconf region automatic err.");
			}

			if (region_automatic == 1) {
				ad->data_region_fmt->sub_desc = (char *)g_strdup(
						_("IDS_ST_BODY_ANSWERINGMODE_AUTOMATIC"));
			} else {
				ad->data_region_fmt->sub_desc =
						setting_phone_lang_get_region_str(
								pa_region);
			}
			G_FREE(ad->data_region_fmt->keyStr);
			ad->data_region_fmt->keyStr = (char *)g_strdup(
					_(REGION_FORMAT_LOWER));
			elm_object_item_data_set(ad->data_region_fmt->item,
					ad->data_region_fmt);
			elm_genlist_item_update(ad->data_region_fmt->item);
		}
		if (ad->data_region_fmt_example) {
			char *example_desc =
					setting_phone_lang_get_example_desc(
							pa_region, ad);
...
#if 0
	if (ad->lang_change) {
		ad->lang_flush_timer = ecore_timer_add(
				0.3,
				setting_phone_language_region_flush_timer_cb,
				ad);
		ad->lang_change = EINA_FALSE;
	}
#endif
