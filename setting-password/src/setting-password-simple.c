		...
		Evas_Object *obj, void *event_info)
{
	...
	/* send result : Cancel */
	add_app_control_reply(&ad->md, "result", "Cancel");
...
static int _handle_step1(void *data, char *collection)
{
	...
	add_app_control_reply(&ad->md, "result", ad->view_type_string);
	SETTING_TRACE("Send Result : %s", ad->view_type_string);

	if (0 == safeStrCmp(ad->view_type_string,
			"SETTING_PW_TYPE_ENTER_LOCK_TYPE"))
		add_app_control_reply(&ad->md, "current", collection);
	...
	if (*step == 0) {	/* Enter new password */
		...
		if (safeStrNCmp(collection1, collection2, 4) == 0) {
			...
			if (SETTING_RETURN_SUCCESS == ret) {
				...
				add_app_control_reply(ad->md, "result",
						ad->view_type_string);
