...
void _popup_resp_cb(void *data, Evas_Object *obj,
		void *event_info)
{
...
void check_attemps_left(void *data)
{
	ret_if(!data);
	SettingPassword *ad = (SettingPassword *)data;
	...
	if (value > 0 && value <= attemps_left_max) {
		...
		display_desc(ad, temp_str, FALSE);
	} else if (value == 0) {
		...
		create_popup_notitle_nobtn(ad, temp, FALSE);
	} else {
...
int check_password(const char *challenge,
...
int set_password(const char *cur_pwd, const char *new_pwd,
		void *data)
{
	SETTING_TRACE_BEGIN;
	...
	/* max attempt count will be handled in password app for a while. */
	if (ad->pw_status == SETTING_PW_STATUS_EMPTY) {
