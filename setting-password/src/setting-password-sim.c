 ...
 *
 ***************************************************/
static void __password_sim_gl_mouse_up(void *data, Evas *e, Evas_Object *obj,
		void *event)
{
	ret_if(!data);
...
static void setting_password_sim_click_softkey_cancel_cb(void *data,
...
static int __create_sim_layout(void *data)
{
	...
	ad->navi_it = elm_naviframe_top_item_get(ad->md.navibar_main);
...
static int setting_password_sim_create(void *cb)
{
	...
	ad->focus_timer = ecore_timer_add(0.5, __setting_password_sim_rotate_cb,
			ad);
...
int setting_password_sim_create_ui(void *data)
{
	...
	/** @todo this function always return 0, not verifying the function
	 * flow with the value */
	return 0;
}

static void setting_password_sim_entry_changed_cb(void *data, Evas_Object *obj,
		void *event_info)
{
	SETTING_TRACE_BEGIN;
	...
	do {
		...
		if (ad->ed_pw2) {
			...
			if (NULL == entry_str || '\0' == entry_str[0]) {
				...
				elm_object_part_text_set(obj, "elm.guide",
						_("IDS_ST_HEADER_ENTER_PIN_ABB3"));
				elm_object_item_signal_emit(ad->ed_pw2->item,
						"elm,state,eraser,hide", "");
				break;
		...
		if (ad->ed_pw3) {
			...
			if (NULL == entry_str || '\0' == entry_str[0]) {
				...
				elm_object_part_text_set(obj, "elm.guide",
						_("IDS_ST_HEADER_ENTER_PIN_ABB3"));
				elm_object_item_signal_emit(ad->ed_pw3->item,
						"elm,state,eraser,hide", "");
				break;
		...
		if (ad->ed_pw1) {
			if (!ad->cur_pwd
					...
					&& (safeStrLen(elm_entry_entry_get(
							ad->ed_pw1->eo_check)) < 8)) {
				SETTING_TRACE_DEBUG(
						"disable flag becomes TRUE");
				disableFlag = TRUE;
			}
		}
	} else {
		if (ad->ed_pw1 && ad->ed_pw1->eo_check && (safeStrLen(
				elm_entry_entry_get(ad->ed_pw1->eo_check)) < 4))
			disableFlag = TRUE;
		if (ad->ed_pw2 && ad->ed_pw2->eo_check && (safeStrLen(
				elm_entry_entry_get(ad->ed_pw2->eo_check)) < 4))
			disableFlag = TRUE;
		if (ad->ed_pw3 && ad->ed_pw3->eo_check && (safeStrLen(
				elm_entry_entry_get(ad->ed_pw3->eo_check)) < 4))
			disableFlag = TRUE;
	}
		...
		void *event_info)
{
	...
	err = haptic_vibrate_file_with_detail(haptic_handle,
			"/usr/share/feedback/haptic/touch/touch.ivt", 1,
			HAPTIC_FEEDBACK_AUTO, HAPTIC_PRIORITY_MIN, NULL);
	...
	if (obj == ad->ed_pw1->eo_check) {
		if (ad->view_type == SETTING_PW_TYPE_PIN_BLOCKED
			...
			if (safeStrLen(elm_entry_entry_get(ad->ed_pw1->eo_check))
				...
				elm_object_focus_set(ad->ed_pw1->eo_check,
						EINA_TRUE);
				return;
			}
		}
...
int setting_password_sim_draw_1line_entry(void *data, void *cb)
{
	...
	case SETTING_PW_TYPE_SIM_LOCK_OFF:
		ad->ed_pw1 = setting_create_Gendial_field_editfield(
				...
				__entry_focused_cb, __entry_unfocused_cb,
				NULL,
...
int setting_password_sim_clear_3line_entry(void *data)
{
	...
	ad->ed_pw2->isFocusFlag = FALSE;
	elm_entry_entry_set(ad->ed_pw2->eo_check, "");
	elm_object_part_text_set(ad->ed_pw2->eo_check, "elm.guide",
			_("Enter PIN"));
	...
	elm_object_part_text_set(ad->ed_pw3->eo_check, "elm.guide",
			_("Enter PIN"));
...
void setting_password_sim_warning_entry_added_byte_popup(SettingPassword *ad,
		int min, int max)
{
...
static void setting_password_sim_done(void *data)
{
	...
	case SETTING_PW_TYPE_PIN_LOCK_ON: {
		...
		if (ret == SETTING_RETURN_SUCCESS) {
			...
			/*TAPI_SIM_PTYPE_PIN1 */
			sim_sec_data.lock_type = TAPI_SIM_LOCK_SC;
			sim_sec_data.pw = (unsigned char *)entry_str;
			sim_sec_data.pw_len = safeStrLen(entry_str);

			tapi_ret = tel_enable_sim_facility(ad->handle,
					&sim_sec_data,
					setting_sim_facility_enable_cb, ad);
			...
			if (TAPI_API_SUCCESS != tapi_ret) {
				...
				/* Success. Send Message to Caller for Modify
				 * Pin Lock slider status */
			}
		} else {
			setting_password_sim_clear_1line_entry(ad);
		}
	...
	case SETTING_PW_TYPE_PIN_LOCK_OFF: {
		...
		if (ret == SETTING_RETURN_SUCCESS) {
			...
			tapi_ret = tel_disable_sim_facility(ad->handle,
					&sim_sec_data,
					setting_sim_facility_disable_cb, ad);
			setting_password_sim_clear_1line_entry(ad);
	...
	case SETTING_PW_TYPE_PIN_BLOCKED: {
		 ...
		 * pin1 code. */
		/* If puk code is correct and success to modify pin1 code,
		 * display pin1 code change popup */
		/* But, if this menu is "Pin Lock", pinlock status will not be
		 * changed. */
		...
		if (ad->cur_pwd == NULL) {
			...
			if (ret == SETTING_RETURN_SUCCESS) {
				...
				tapi_ret = tel_verify_sim_puks(ad->handle,
						&puk1_data, &pin1_data,
						setting_sim_verify_puks_cb, ad);
				FREE(ad->cur_pwd);
				ad->cur_pwd = NULL;
				...
				if (TAPI_API_SUCCESS != tapi_ret) {
					...
					/* Success. Send Message to Caller for
					 * Modify Pin Lock slider status */
	...
	case SETTING_PW_TYPE_PIN2_BLOCKED: {
		...
		if (ret == SETTING_RETURN_SUCCESS) {
			...
			tapi_ret = tel_verify_sim_puks(ad->handle, &puk2_data,
					&pin2_data, setting_sim_verify_puks_cb,
					ad);
	...
	case SETTING_PW_TYPE_CHANGE_PIN: {
		...
		if (ad->cur_pwd) {
			...
			if (ret == SETTING_RETURN_SUCCESS) {
				...
				tapi_ret = tel_change_sim_pins(ad->handle,
						&sim_old_sec_data,
						&sim_new_sec_data,
						setting_sim_change_pins_cb, ad);
				setting_password_sim_clear_2line_entry(ad);

				if (TAPI_API_SUCCESS != tapi_ret) {
					ad->focus_data = ad->ed_pw1;
					SETTING_TRACE_DEBUG(
							"%s*** [ERR] TelTapiSimChangePIN err=%d ***%s",
							SETTING_FONT_RED,
							tapi_ret,
							SETTING_FONT_BLACK);
					setting_password_ug_create_popup_notitle_nobtn(
	...
	case SETTING_PW_TYPE_CHANGE_PIN2: {
		...
		case SETTING_PW_TYPE_SIM_LOCK_ON: {
			...
			if (ret == SETTING_RETURN_SUCCESS) {
				...
				/*TAPI_SIM_PTYPE_SIM */
				sim_sec_data.lock_type = TAPI_SIM_LOCK_PS;
				sim_sec_data.pw = (unsigned char *)entry_str;
				sim_sec_data.pw_len = safeStrLen(entry_str);

				tapi_ret = tel_enable_sim_facility(ad->handle,
						&sim_sec_data,
						setting_sim_facility_enable_cb,
						ad);
				setting_password_sim_clear_2line_entry(ad);
				...
				if (TAPI_API_SUCCESS != tapi_ret) {
					ad->focus_data = ad->ed_pw1;
					SETTING_TRACE_DEBUG(
							"%s*** [ERR] tel_enable_sim_facility err=%d ***%s",
							SETTING_FONT_RED,
							tapi_ret,
							SETTING_FONT_BLACK);
					setting_password_ug_create_popup_notitle_nobtn(
		...
		case SETTING_PW_TYPE_SIM_LOCK_OFF: {
			...
			if (ret == SETTING_RETURN_SUCCESS) {
				...
				tapi_ret = tel_disable_sim_facility(ad->handle,
						&sim_sec_data,
						setting_sim_facility_disable_cb,
						ad);
				setting_password_sim_clear_1line_entry(ad);
		...
		case SETTING_PW_TYPE_FDN_MODE_ON: {
			...
			if (ret == SETTING_RETURN_SUCCESS) {
				...
				/*TAPI_SIM_PTYPE_PIN2; */
				sim_sec_data.lock_type = TAPI_SIM_LOCK_FD;
				sim_sec_data.pw_len = safeStrLen(entry_str);
				sim_sec_data.pw = (unsigned char *)entry_str;
...
void setting_get_sim_lock_info_cb(TapiHandle *handle, int result, void *data,
		void *user_data)
{
	SETTING_TRACE_BEGIN;
...
void setting_get_pin_lock_info_cb(TapiHandle *handle, int result, void *data,
		void *user_data)
{
	SETTING_TRACE_BEGIN;
	...
	if (ad->view_type == SETTING_PW_TYPE_CHANGE_PIN) {
		...
		if (ad->incorrect_pin1_flag) {
			if (lock->retry_count > 1) {
				snprintf(rtemp, SETTING_STR_SLP_LEN, "%s. %s.",
						_("IDS_ST_BODY_INCORRECT_PIN_E"),
						_(ATTEMPTS_DESC));
				snprintf(temp, SETTING_STR_SLP_LEN, rtemp,
						lock->retry_count);
			} else {
				snprintf(rtemp, SETTING_STR_SLP_LEN, "%s. %s.",
						_("IDS_ST_BODY_INCORRECT_PIN_E"),
			...
			if (lock->retry_count > 1)
				snprintf(temp, 256, _(ATTEMPTS_DESC),
						lock->retry_count);
			else if (lock->retry_count > 0)
				snprintf(temp, 256, _(ATTEMPT_DESC),
						lock->retry_count);
		}
	} else if (ad->view_type == SETTING_PW_TYPE_CHANGE_PIN2) {
		char rtemp[SETTING_STR_SLP_LEN] = { 0, };
...
void setting_sim_change_pins_cb(TapiHandle *handle, int result, void *data,
		void *user_data)
{
	SETTING_TRACE_BEGIN;
	...
	SETTING_TRACE_SECURE_DEBUG("sim_event_data->type:%d",
			sim_event_data->type);
	SETTING_TRACE_SECURE_DEBUG("sim_event_data->retry_count[%d]",
	...
	if (TAPI_SIM_PTYPE_PUK1 == sim_event_data->type) {
		...
		if (TAPI_SIM_PIN_OPERATION_SUCCESS == sec_rt) {
			...
			safeCopyStr(ad->view_type_string,
					...
					sizeof("SETTING_PW_TYPE_PIN1_UNBLOCKED") + 1);
			...
			if (sim_event_data->retry_count <= 0) {
				...
				result_info.stat = SIM_INCORRECT_PUK1_CODE;
				result_info.retry_cnt =
						sim_event_data->retry_count;
				ad->puk_verified_flag = TRUE;
				setting_view_update(&setting_view_password_main,
						ad);
			}
		...
		SettingPassword *ad)
{
	...
	case SIM_PIN_LOCK_OFF_SUCCESS: {
		if (ad->view_type == SETTING_PW_TYPE_SIM_LOCK_OFF) {
	...
	case SIM_INCORRECT_PIN1_CODE: {
		...
		if (result->retry_cnt > 1) {
			/* there has '%d' in
			 * 'IDS_ST_POP_INCORRECT_PIN_PD_ATTEMPTS_LEFT',
			 * must transform */
			char tmp_str2[SETTING_STR_SLP_LEN] = { 0, };
			snprintf(tmp_str2, SETTING_STR_SLP_LEN, "%s. %s.",
					_("IDS_ST_BODY_INCORRECT_PIN_E"),
					_(ATTEMPTS_DESC));
	...
	case SIM_INCORRECT_PUK1_CODE: {
		...
		if (result->retry_cnt > 1) {
			/* there has '%d' in
			 * 'IDS_ST_POP_INCORRECT_PUK1_PD_ATTEMPTS_LEFT',
			 * must transform */
			int ret = snprintf(tmp_str, SETTING_STR_SLP_LEN,
					PUK1_ERR_DESC, result->retry_cnt);
			setting_retm_if(ret < 0, "snprintf fail");
			ret_str = tmp_str;
		} else {
